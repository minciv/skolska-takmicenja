...
class BestStudentReportFrame(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        self.control_frame = ttk.LabelFrame(main_frame, text="Параметри извештаја", padding=10)
        self.control_frame.pack(fill=X, expand=NO, pady=(0, 10))
        
        # Избор школске године
        ttk.Label(self.control_frame, text="Школска година:").grid(row=0, column=0, sticky=W, pady=5, padx=5)
        self.school_year_cb = ttk.Combobox(self.control_frame, textvariable=self.school_year_var, width=20)
        self.school_year_cb.grid(row=0, column=1, sticky=W, pady=5, padx=5)
        
        # Дугмад за акције
        self.button_frame = ttk.Frame(self.control_frame)
        self.button_frame.grid(row=0, column=2, padx=20)
        ttk.Button(self.button_frame, text="Прикажи ранг листу", command=self.show_ranking).pack(side=LEFT, padx=5)
        ttk.Button(self.button_frame, text="Извези у Excel", command=self.export_to_excel).pack(side=LEFT, padx=5)
        ...
        # Креирамо и сачувајмо референцу на контејнер за резултате
        self.result_container = ttk.Frame(main_frame)
        self.result_container.pack(fill=BOTH, expand=YES)
        
        # Иницијално празна табела
        self.result_frame = ttk.LabelFrame(self.result_container, text="Ранг листа", padding=10)
    ...
    def calculate_student_scores(self, school_year):
        ...
        try:
            ...
            # Филтрирамо само ученике који су у 8. разреду у датој школској години
            students = []
            
            # Користимо директан приступ функцији за израчунавање разреда уместо креирања нове инстанце
            # Функција за израчунавање разреда на основу почетног разреда и школске године
            def calculate_grade(grade, student_school_year, target_school_year):
                if student_school_year == target_school_year:
                    return grade
                
                # Рачунамо разлику у годинама
                try:
                    student_year = int(student_school_year.split('/')[0])
                    target_year = int(target_school_year.split('/')[0])
                    year_diff = target_year - student_year
                    return grade + year_diff
                except:
                    return grade
            
            for student in all_students:
                student_id, student_name, class_enrolled, grade, student_school_year = student
                
                # Провера да ли је ученик у 8. разреду у траженој школској години
                current_grade = calculate_grade(grade, student_school_year, school_year)
                
                if current_grade == 8:
                    students.append((student_id, student_name, class_enrolled))
    ...
    def show_ranking(self):
        ...
        try:
            # Проверавамо да ли је интерфејс и даље валидан
            try:
                school_year = self.school_year_var.get()
            except:
                # Ако није, поново креирамо интерфејс
                messagebox.showinfo("Обавештење", "Поновно постављамо интерфејс...")
                self.regenerate_ui()
                return
                
            # Провера улазних параметара
            if not school_year:
                messagebox.showwarning("Упозорење", "Молимо изаберите школску годину.")
                return
                
            # Провера да ли контејнер резултата постоји
            if not hasattr(self, 'result_container') or not self.result_container.winfo_exists():
                # Поновно креирамо интерфејс и обавештавамо корисника
                messagebox.showinfo("Обавештење", "Регенеришемо интерфејс...")
                self.regenerate_ui()
                return
                
            # Уклањамо старе резултате
            for widget in self.result_container.winfo_children():
                widget.destroy()
                
            # Креирамо нови фрејм за резултате
            self.result_frame = ttk.LabelFrame(self.result_container, text="Ранг листа - Школска година: " + school_year, padding=10)
            self.result_frame.pack(fill=BOTH, expand=YES)
            
        except Exception as e:
            # У случају грешке, покушавамо са регенерацијом интерфејса
            messagebox.showerror("Грешка", f"Грешка при припреми приказа ранг листе: {str(e)}. Регенеришемо интерфејс...")
            self.regenerate_ui()
            return
        ...
        for student in student_scores:
            student_frame = ttk.LabelFrame(
                details_frame, 
                text=f"{student['rank']}. {student['student_name']} - Укупно бодова: {student['total_points']}",
                padding=5
            )
    ...
    # Помоћна метода за очување навигације
    def regenerate_ui(self):
        """Поновно креирање комплетног интерфејса да би се обезбедила навигација"""
        # Запамтимо тренутну школску годину
        current_school_year = self.school_year_var.get() if hasattr(self, 'school_year_var') else ""
        
        # Уклањамо све виџете из овог фрејма
        for widget in self.winfo_children():
            widget.destroy()
        
        # Поново постављамо све варијабле
        self.school_year_var = StringVar(value=current_school_year)
        
        # Поново креирамо интерфејс
        self.setup_ui()
        
        # Поново учитавамо школске године
        self.load_school_years()
        
        # Ако је била изабрана школска година, постављамо је поново
    ...
    def export_to_excel(self):
        ...
        try:
            ...
            ws_ranking.append([])
            ws_ranking.append(["Ранг", "Ученик", "Број успеха", "Укупно бодова"])
